...
Ehrlich benannt: **Ollama ist kein Bestandteil dieser Installation.**
`settings.llm_provider` unterstützt aktuell ausschließlich "anthropic"
(siehe app/config/settings.py, ARCHITECTURE.md §10 - lokales LLM via
Ollama ist eine bewusst weiterhin offene, nicht getroffene Entscheidung).
Diese Seite zeigt das deshalb als expliziten Hinweis statt einen
Verbindungsstatus zu einem Dienst vorzutäuschen, der gar nicht angebunden
ist.

Die Claude-API-Erreichbarkeitsprüfung ist bewusst NICHT Teil des normalen
Seitenaufrufs (kein automatischer Aufruf bei jedem Laden der
Systemstatus-Seite) - sie kostet zwar laut Anthropic-Dokumentation keine
Token (`GET /v1/models` ist ein reiner Metadaten-Abruf), aber ein
automatischer Hintergrundaufruf bei jedem Seitenaufruf wäre trotzdem eine
unnötige, unkontrollierte ausgehende Netzwerkverbindung - konsistent mit
der übrigen Kostenkontroll-Disziplin des Projekts (Prompt 33) wird sie nur
auf ausdrücklichen Admin-Klick ausgeführt (siehe
app/web/monitoring_router.py: check_api_reachability)."""
